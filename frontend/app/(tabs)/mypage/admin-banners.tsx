// 관리자 - 배너 관리 (GET /api/admin/banners)
...
export default function AdminBannersScreen() {
  ...
  const [list, setList] = useState<adminService.BannerItem[]>([]);
  ...
  const load = useCallback(async (refresh = false, nextPage?: number) => {
    ...
    try {
      ...
      const res = await adminService.getAdminBannerList({ page: p, size: 20 });
      const newContent = res.content ?? [];
      setList((prev) => (refresh || p === 0 ? newContent : [...prev, ...newContent]));
      setTotalPages(res.totalPages ?? 0);
      setPage(res.number ?? 0);
    } catch (e) {
      setError(e instanceof Error ? e.message : '배너 목록 조회 실패');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleToggle = async (item: adminService.BannerItem) => {
    try {
      await adminService.toggleBannerActive(item.id);
      load(true);
    } catch (e) {
  ...
  const handleDelete = (item: adminService.BannerItem) => {
    Alert.alert('삭제 확인', `"${item.title}" 배너를 삭제할까요?`, [
      { text: '취소', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await adminService.deleteBanner(item.id);
            load(true);
          } catch (e) {
  ...
  return (
    <View style={styles.container}>
            ...
            <View key={item.id} style={styles.card}>
              ...
              <View style={styles.cardBody}>
                <View style={styles.cardHeader}>
                  ...
                  <Text style={styles.linkType}>{item.linkType}</Text>
                </View>
                ...
                <View style={styles.actions}>
                  ...
                  <Pressable
                    ...
                    onPress={() => router.push({ pathname: '/(tabs)/mypage/admin-banner-edit' as any, params: { id: String(item.id) } })}
                  >
      ...
      <Pressable
        style={styles.fab}
        onPress={() => router.push('/(tabs)/mypage/admin-banner-edit' as any)}
      >

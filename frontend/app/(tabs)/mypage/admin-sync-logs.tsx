...
export default function AdminSyncLogsScreen() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}
      ...
      {content.length === 0 ? <Text style={styles.empty}>동기화 로그가 없습니다.</Text> : content.map((log) => (
        <Pressable key={log.id} style={styles.card}
          onPress={() => router.push({ pathname: "/(tabs)/mypage/admin-sync-log-detail", params: { logId: String(log.id) } })}>
          <View style={styles.cardBody}>
            <Text style={styles.source}>{log.sourceType} · {log.status}</Text>

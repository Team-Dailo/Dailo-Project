...
export default function BoardChatListScreen() {
  ...
  const goRoom = useCallback(
    (room: ChatRoomRow) => {
      ...
      // ✅ 문자열 조립(push('/xxx?aa=..')) 하지 말고 object로!
      // ✅ 여기 pathname은 네 프로젝트의 채팅방 화면 파일명에 맞춰야 함.
      
      router.push({
        pathname: "/board/char-room", // ⚠️ 실제 파일명이 chat-room이면 "/board/chat-room"로 바꿔
        params: {
          roomId: item.id,
          userId: myId,
          name: item.name,
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
      ...
      {useMock ? (
        <Text style={styles.banner}>서버 연결 실패로 mockChats를 표시 중</Text>
      ) : null}
